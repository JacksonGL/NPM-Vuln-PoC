...
cd directory-traversal/node-server-forfront
node replace.js `pwd` > config.js
cp config.js ../../node_modules/node-server-forfront/lib/config/
...
# start the server
echo -e "\t[${GREEN}start vulnerable server${NC}]: ${BLUE}node-server-forfront${NC}"
